...
public class EpubPositionsService(
    ...
    public sealed class ReflowableStrategy {
        ...
        public companion object {
            /**
             * Recommended strategy: archive entry length split by [pageLength] bytes.
             *
             * 512 bytes (compressed) roughly maps to one rendered screen page for CJK
             * content, which makes the per-chapter position count track the actual
             * number of pages far more closely than the historical 1024-byte heuristic.
             */
            public val recommended: ReflowableStrategy = ArchiveEntryLength(pageLength = 512)
        }
    }
