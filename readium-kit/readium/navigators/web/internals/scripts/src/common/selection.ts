...
export class SelectionManager {
  ...
  // Programmatic selection from a point (long press replacement)
  selectAtPoint(x: number, y: number): Selection | null {
    const range = this.window.document.caretRangeFromPoint(x, y)
    if (!range) {
      return null
    }
    try {
      ...
      // expand('word') is non-standard but supported in Chromium WebView
    }
  ...
  // Get the bounding rect of the start handle position
  getStartHandleRect(): Rect | null {
    const selection = this.window.getSelection()!
    ...
    return dezoomRect(domRectToRect(rect), zoom)
  }

  // Get the bounding rect of the end handle position
  getEndHandleRect(): Rect | null {
    const selection = this.window.getSelection()!
    if (selection.rangeCount === 0) return null
    const range = selection.getRangeAt(0).cloneRange()
  ...
  // Extend the start of the selection to a new point
  extendStart(x: number, y: number): Selection | null {
    const selection = this.window.getSelection()!
    if (selection.rangeCount === 0) return null
    const range = selection.getRangeAt(0)
    const newRange = this.window.document.caretRangeFromPoint(x, y)
    if (!newRange) return null
    // Keep focusNode/offset, change anchor
    selection.removeAllRanges()
    const ordered = createOrderedRange(
      newRange.startContainer, newRange.startOffset,
      range.endContainer, range.endOffset
    )
    if (!ordered || ordered.collapsed) return null
    selection.addRange(ordered)
    return this.getCurrentSelection()
  }

  // Extend the end of the selection to a new point
  extendEnd(x: number, y: number): Selection | null {
    const selection = this.window.getSelection()!
    if (selection.rangeCount === 0) return null
    const range = selection.getRangeAt(0)
    const newRange = this.window.document.caretRangeFromPoint(x, y)
    if (!newRange) return null
    // Keep anchorNode/offset, change focus
    selection.removeAllRanges()
    const ordered = createOrderedRange(
